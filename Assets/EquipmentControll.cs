...
public class EquipmentControll : MonoBehaviour
{
    ...
    IEnumerator WeaponMove(Item _item)
    {
        ...
        while (true)
        {
            ...
            {
                _weaponImage.GetComponent<RectTransform>().position = Vector3.MoveTowards(_weaponImage.GetComponent<RectTransform>().position, targetPos, 10f);
    ...
    IEnumerator ArmorMove(Item _item)
    {
        ...
        while (true)
        {
            if (Vector3.Distance(tempPos, targetPos) > 0.1f)
            {
                _armorImage.GetComponent<RectTransform>().position = Vector3.MoveTowards(_armorImage.GetComponent<RectTransform>().position, targetPos, 10f);
    ...
    IEnumerator ShovelMove(Item _item)
    {
        ...
        while (true)
        {
            if (Vector3.Distance(tempPos, targetPos) > 0.1f)
            {
                _shovelImage.GetComponent<RectTransform>().position = Vector3.MoveTowards(_shovelImage.GetComponent<RectTransform>().position, targetPos, 10f);
    ...
    IEnumerator PotionMove(Item _item)
    {
        ...
        while (true)
        {
            if (Vector3.Distance(tempPos, targetPos) > 0.1f)
            {
                _potionImage.GetComponent<RectTransform>().position = Vector3.MoveTowards(_potionImage.GetComponent<RectTransform>().position, targetPos, 10f);
